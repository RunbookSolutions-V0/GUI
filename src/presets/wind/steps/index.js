export default {
  ...
  action: ({ props, context, state }) => ({
    class: [
      ...
      'after:border-t-2',
      {
        'after:border-background-200 after:dark:border-background-700':
  ...
  step: ({ context, props, state }) => ({
    class: [
      ...
      // Colors
      {
        'text-background-400 dark:text-white/60':
          !context.active && state.d_activeStep < context.index,
        'border-background-100 dark:border-background-700':
          !context.active && state.d_activeStep < context.index,
      ...
      // States
      {
        'hover:border-background-300 dark:hover:border-background-500':
  ...
  label: ({ context }) => ({
    class: [
      ...
      // Colors
      {
        'text-background-400 dark:text-white/60': !context.active,
        'text-background-800 dark:text-white/80': context.active
      },
