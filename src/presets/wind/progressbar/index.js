export default {
  root: ({ props }) => ({
    class: [
      ...
      {
        'h-2 bg-background-100 dark:bg-background-700 ':
          props.mode == 'indeterminate' || !props.showValue
      },

      // Before & After (!indeterminate)
      {
        'before:absolute before:w-full before:rounded-md before:h-2 before:top-[1.25rem] before:left-0 before:bottom-0 before:bg-background-100 dark:before:bg-background-700':
