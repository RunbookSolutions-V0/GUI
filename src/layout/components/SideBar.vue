    ...
    >
      <template #start>
        <span class="inline-flex items-center justify-center gap-1 px-2 py-2 w-full sm:w-[15rem]">
          <img v-if="settings.darkMode" src="../../assets/logo-dark.svg" class="h-24" alt="Logo" />
          <img v-if="!settings.darkMode" src="../../assets/logo.svg" class="h-24" alt="Logo" />
        </span>
      </template>
      ...
      <template #item="{ item, props }">
        <PVPanelMenu :model="[item]" v-if="item.items" />
        <template v-else>
